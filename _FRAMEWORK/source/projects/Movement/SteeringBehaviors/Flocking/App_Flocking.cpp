...
{	
	...
	SAFE_DELETE(m_pWanderForAgentToEvade);
}
...
void App_Flocking::Start()
{
	...
	m_pWanderForAgentToEvade = new Wander();
	m_pAgentToEvade = new SteeringAgent();
	m_pAgentToEvade->SetSteeringBehavior(m_pWanderForAgentToEvade);
	m_pAgentToEvade->SetAutoOrient(true);
	m_pAgentToEvade->SetBodyColor({ 1,0,0 });
	m_pAgentToEvade->SetLinearVelocity(m_pAgentToEvade->GetLinearVelocity() * 200.f); // double the speed at startup
	m_pAgentToEvade->SetPosition({ m_TrimWorldSize / 2.f, m_TrimWorldSize / 2.f });
	
	// to see debug of m_pAgentToEvade...
