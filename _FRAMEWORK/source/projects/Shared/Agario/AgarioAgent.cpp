...
void AgarioAgent::SetToPursue(Elite::Vector2 pursuePos)
{
	m_pPursue->SetTarget(pursuePos);
	SetSteeringBehavior(m_pPursue);
}

void AgarioAgent::ChangeEvadeRadius(float newRadius)
{
	m_pFlee->SetFleeRadius(newRadius);
	m_pEvade->SetEvadeRadius(newRadius);
}
