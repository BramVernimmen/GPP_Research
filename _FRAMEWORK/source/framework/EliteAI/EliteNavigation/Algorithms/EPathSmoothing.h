	...
	public:
		...
		static std::vector<Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int portalIdx = 1; portalIdx < amtPortals; ++portalIdx)
			{
				...
				if (newRightLeg.Cross(rightLeg) <= 0.0f) // if smaller than 0 -> clockwise
				{
					//2. See if new line degenerates a line segment - RIGHT
					if (newRightLeg.Cross(leftLeg) >= 0.0f) // not crossing the left leg
					{
						rightLeg = newRightLeg;
						rightLegIdx = portalIdx;
					}
					else // crossing the left leg
					{
						// move apex
						apexPos += leftLeg;
						// set apexIndex to leftLegIdx
						apexIdx = leftLegIdx;
						portalIdx = leftLegIdx + 1; // next portal check
				...
				if (newLeftLeg.Cross(leftLeg) >= 0.0f)
				{
					//2. See if new line degenerates a line segment - LEFT
					if (newLeftLeg.Cross(rightLeg) <= 0.0f) // not crossing the right leg
					{
						leftLeg = newLeftLeg;
						leftLegIdx = portalIdx;
					}
					else // crossing the right leg
					{
						// move apex
						apexPos += rightLeg;
						// set apexIndex to rightLegIdx
						apexIdx = rightLegIdx;
						portalIdx = rightLegIdx + 1; // next portal check
