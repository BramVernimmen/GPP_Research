...
namespace Elite
{
	...
	inline std::vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		if (currentIdx == invalid_node_index) // no node with uneven connections have been found
		...
		{
			if (graphCopy->GetNodeConnections(currentIdx).size() > 0) // node has neighbors
			{
			...
			else
			{
				...
				// take the last node in the stack as the current
